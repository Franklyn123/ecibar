...
class Asistencia extends React.Component {
  ...
  componentDidMount() {
    ...
    } else if (loguedUsername !== 'octavio' && loguedUsername !== 'administracion') {
    ...
    return (
      <div>
        ...
        <PapperBlock title="" desc="">
          {alumnos.map(exp => (
            <Paper className={props.classes.paper} style={{ marginTop: 8 }} key={exp.get('numeracion')}>
              <Grid container spacing={16}>
                <Grid item>
                  <ButtonBase className={props.classes.image}>
                    <img className={props.classes.img} alt="complex" src="/images/usuario.png" />
                  </ButtonBase>
                </Grid>
                <Grid item xs={12} sm container>
                  <Grid item xs container direction="column" spacing={16}>
                    <Grid item xs>
                      ...
                      <Typography color="textSecondary">{exp.get('alumno').get('dni')}</Typography>
                      <Typography gutterBottom variant="subtitle1">
                        Apellidos y Nombres:
                      </Typography>
