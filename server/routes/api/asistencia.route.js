        ...
        if (dias_manejo > 0) {
            if (dias_manejo > expediente.asistencias_manejo.length) {
                if (expediente.vehiculo == null && expediente.instructor == null) {
                    ...
                    expediente = await Expediente.findOneAndUpdate({ _id: ObjectId(expediente._id) }, {
                            $set: {
                ...
                let instructor = await Instructor.findOne({ _id: expediente.instructor });
                ...
                await Vehiculo.findOneAndUpdate({ _id: expediente.vehiculo }, {
                        $set: {
                            km: String(kilometrajeAumentado.toString())
                        }
                    }, {},
                ...
                // Obtener la ultima leccion de hoy realizada con el vehiculo 
                var hora_cero = moment('00:00', 'hh:mm').format();
                console.log(hora_cero);
                var hora_24 = moment('23:59', 'hh:mm').format();
                console.log(hora_24);
                
                let ultima_leccion = await Leccion.findOne({ $and: [ 
                    { 
                        fecha_fin: 
                        { 
                            $gte: hora_cero, 
                ...
                //return res.json(ultima_leccion);

                // Constante
                const INTERVALO_MINUTOS = 50;

                var fecha_inicio = moment('08:00', 'hh:mm');
                var fecha_fin = moment(fecha_inicio, "hh:mm")
                                .add(INTERVALO_MINUTOS, 'minutes')
                                .format();
                ...
                if(ultima_leccion != null) {

                    fecha_inicio = ultima_leccion.fecha_fin;
                    fecha_fin = moment(fecha_inicio, "hh:mm")
                                .add(INTERVALO_MINUTOS, 'minutes')
                                .format();

                    leccion.fecha_inicio = fecha_inicio;
                    leccion.fecha_fin = fecha_fin;
                }
                ...
                if (expediente.asistencias_manejo.length == 0) {
                    expediente.fecha_inicio_manejo = fecha_inicio;
                    expediente.km_inicio = Number(kilometrajeVehiculo);
                }

                // Dia intermedio
                expediente.asistencias_manejo.push(leccion._id);

                // Ultimo dia
                if (expediente.asistencias_manejo.length == dias_manejo) {
                    expediente.fecha_fin_manejo = fecha_inicio;
...
router.get('/generar/:id', async(req, res) => {
    try {
        ...
        for (
            var m = moment(fecha_registro); m.diff(fecha_fin_teoria, 'days') <= 0; m.add(1, 'days')
        ) {
            if (m.isoWeekday() !== 7) {
        ...
        if (
            ...
            for (let i = 0; i < clases_vehiculos.length; i++) {
                ...
                for (let j = 0; j < vehiculos.length; j++) {
                    ...
                    for (
                        var m = moment(fecha_inicio_consulta); m.diff(fecha_fin_consulta, 'days') <= 0; m.add(1, 'days')
                    ) {
                        let hora_0 = m
                            .clone()
                            .hour(-5)
...
router.get('/test/', async(req, res) => {
    ...
    try {
        ...
        let expediente = await Expediente.findOne({ numeracion: 4040 });
