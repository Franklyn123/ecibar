...
router.get('/provincias/:id', (req, res) => {
    Provincia.find({region_id:req.params.id})
    .then(provincias => res.json(provincias));
});
router.get('/distritos/:id', (req, res) => {
    Distrito.find({region_id:req.params.id})
    .then(distritos => res.json(distritos));
});
...
router.get('/estado', async (req, res) => {
    const num = await Expediente.find().sort({"numeracion": -1}).limit(1);
    res.json({numeracion:num[0].numeracion+1});
});
router.get('/alumnos', async (req, res) => {
    res.json([{nombres: "Cahuana Rojas, Frankyln",dni:"73464646"},{nombres: "Avila Cordava, Aaron",dni:"45454646"},{nombres: "Rengifo Garcia, Bragan",dni:"89566646"}]);
});
router.post('/ficha_inscripcion', async (req, res) => {
    try{
        ...
        if(cantidadAlumnos==0){
            ...
            await Alumno.updateOne({dni:req.body.dni},{$set:{
                    a_paterno: req.body.aPaterno,
                    a_materno: req.body.aMaterno,
                    sexo: req.body.sexo,
                    domicilio: req.body.domicilio,
