...
router.post('/expediente', async(req, res) => {
    ...
    if (expedientes.length > 0) {
        ...
        // Consultar el atributo de asistencias de manejo de expediente
        var dias_asistencia_manejo = await Expediente.findOne({ _id: expedientes[0]._id }, { asistencias_manejo: 1 });

        // Consultar cada elemento de asistencias de manejo
        var lecciones = await Leccion.find({
            _id: { $in: dias_asistencia_manejo['asistencias_manejo'] }
        });

        // Asignar las lecciones como nuevas asistencias de manejo
        expedientes[0].asistencias_manejo = lecciones;
